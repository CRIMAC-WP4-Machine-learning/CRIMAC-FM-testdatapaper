...
def TSf(
    ...
    mean_relative_amplitude_pre_peak = np.mean(np.abs(pre_peak)/np.abs(peak_value[:,None]),axis=1)
    mean_relative_amplitude_post_peak = np.mean(np.abs(post_peak)/np.abs(peak_value[:,None]),axis=1)
    max_relative_amplitude_pre_peak = np.max(np.abs(pre_peak)/np.abs(peak_value[:,None]),axis=1)
    max_relative_amplitude_post_peak = np.max(np.abs(post_peak)/np.abs(peak_value[:,None]),axis=1)
...
def pc2tsf(trackdir: str, ncdir: str, indexdir: str, CTDdir: str, outputdir: str, FFTdir: str, workfiledir: str)->list:
    ...
    for trackfile in trackfiles:
        ...
        for i, freq in enumerate(freqs_targets):
            ...
            signal_to_noise_ratio_tot = construct_full_TSf(signal_to_noise_ratio,freq_tot,f_m[0])
            ...
            output_temp = xr.Dataset(
                {
                    ...
                    'SNR': (['i', 'frequency'], signal_to_noise_ratio_tot),
